 ...
 * 1-wide BOOM.
 */
class WithNSmallBoomsSW(n: Int = 1, overrideIdOffset: Option[Int] = None) extends Config(
  new WithSWBPD ++
  new Config((site, here, up) => {
    case TilesLocated(InSubsystem) => {
      val prev = up(TilesLocated(InSubsystem), site)
 ...
 * 1-wide BOOM.
 */
class WithNSmallBoomsPerceptron(n: Int = 1, overrideIdOffset: Option[Int] = None) extends Config(
  new WithPerceptronBPD ++
  new Config((site, here, up) => {
    case TilesLocated(InSubsystem) => {
      val prev = up(TilesLocated(InSubsystem), site)
      val idOffset = overrideIdOffset.getOrElse(prev.size)
...
class WithPerceptronBPD extends Config((site, here, up) => {
  case TilesLocated(InSubsystem) => up(TilesLocated(InSubsystem), site) map {
    case tp: BoomTileAttachParams => tp.copy(tileParams = tp.tileParams.copy(core = tp.tileParams.core.copy (
      bpdMaxMetaLength = 1,
      globalHistoryLength = 64,
      ...
      localHistoryNSets = 0,
      branchPredictor = ((resp_in: BranchPredictionBankResponse, p: Parameters) => {
        val perceptronNum = 40
        val perc = Module(new PerceptronBranchPredictorBank(perceptronNum)(p))
        perc.io.resp_in(0) := resp_in
        (Seq(perc), perc.io.resp)
      })
    )))
    case other => other
  }
